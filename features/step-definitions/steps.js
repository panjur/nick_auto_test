...
Then(/^I click on Menu screen$/, () => {
  SignUpPage.clickMenu();
});

Then(/^I click on Account link$/, () => {
  SignUpPage.clickAccount();
});

Then(/^I click on Membership Cancellation button$/, () => {
  SignUpPage.clickOnCancelMembership();
});

Then(/^I select the cancellation reason$/, () => {
  SignUpPage.selectMembershipCancelReason();
  SignUpPage.clickContinue();
});

Then(/^I click on Account Cancellation buuton$/, () => {
  SignUpPage.clickOnAccountCancellation();
});

Then(/^I redirect to the login screen$/, () => {
  expect(browser.$('[name="email"]')).toBeDisplayed();
  expect(browser.$('[name="password"]')).toBeDisplayed();
  expect(browser.$('input[type="submit"]')).toBeDisplayed();
});
